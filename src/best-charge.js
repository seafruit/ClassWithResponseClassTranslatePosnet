...
class Translator {
  ...
  digitalToBarcode(digital) {
    ...
    let returnInformation = translator.checkDigitalFormat(digital);
    if (returnInformation.type === true) {
      let allItems = allItem();
      let formattedDigital = translator.dropHypken(digital);
      let digitalArray = translator.digtialIntoArray(formattedDigital);
      let digitalAndCheckCode = translator.calculateCheckCode(digitalArray);
      let barcodes = translator.matchBarcode(digitalAndCheckCode, allItems);
      let barcodeString = translator.addStartAndEnd(barcodes);
  ...
  tanslateBarcode(input) {
    ...
    let allItems = allItem();
    let isFormattedBarcode = translate.checkBarcodeFormat(input);
    console.log(isFormattedBarcode);
    if (isFormattedBarcode.type) {
      // debugger;
      let formatBarcode = translate.formattedBarcode(input);
      let barcodeArray = translate.barcodeIntoArrar(formatBarcode);
      let digitals = translate.matchDigital(barcodeArray, allItems);
      let isRight = translate.isTrueCheckCode(digitals);
      if (!isRight) return false
      let digital = translate.addHypken(digitals);
      return {digital, type: true}
    } else {
